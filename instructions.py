...
class Instructions:
    def __init__(self, text, font, timer=None):
        self.font = font
        self.timer = timer
        self.lines = wrap_text(text, font, configs.window.WIDTH - 200)
        self.hint_font = pygame.font.Font(configs.font.ROBOTO_MONO_REGULAR, 13)
        self.visible = True
    ...
    def draw(self, screen):
        ...
        screen.blit(hint, (configs.window.WIDTH // 2 - hint.get_width() // 2, configs.window.HEIGHT - 60))

    def draw_hint(self, screen):
        hint = self.hint_font.render("I = instruções", True, COLORS["grey"])
        screen.blit(hint, (configs.window.WIDTH // 2 - hint.get_width() // 2, configs.window.HEIGHT - 28))
